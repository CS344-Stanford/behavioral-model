...
class SimpleSumeSwitchHandler : virtual public SimpleSumeSwitchIf {
 public:
  explicit SimpleSumeSwitchHandler(SimpleSumeSwitch *sw)
    : switch_(sw) { }
...
//    return -1;
  ...
  int32_t set_egress_queue_depth(const int32_t port_num,
    ...
    return 0;//TODO: //switch_->set_egress_queue_depth(port_num,
  ...
  int32_t set_all_egress_queue_depths(const int32_t depth_pkts) {
    ...
    return 0;// TODO: //switch_->set_all_egress_queue_depths(
        //static_cast<uint32_t>(depth_pkts));
  }
  ...
  int32_t set_egress_queue_rate(const int32_t port_num,
    ...
    return 0;//TODO: //switch_->set_egress_queue_rate(port_num,
  ...
  int32_t set_all_egress_queue_rates(const int64_t rate_pps) {
    ...
    return 0;//TODO:// switch_->set_all_egress_queue_rates(static_cast<uint64_t>(rate_pps));
  }

  int64_t get_time_elapsed_us() {
    bm::Logger::get()->trace("get_time_elapsed_us");
    // cast from unsigned to signed
    return 0;//TODO:static_cast<int64_t>(switch_->get_time_elapsed_us());
  }

  int64_t get_time_since_epoch_us() {
    bm::Logger::get()->trace("get_time_since_epoch_us");
    // cast from unsigned to signed
    return 0;//TODO:static_cast<int64_t>(switch_->get_time_since_epoch_us());
  }

 private:
  SimpleSumeSwitch *switch_;
};

boost::shared_ptr<SimpleSumeSwitchIf> get_handler(SimpleSumeSwitch *sw) {
  return boost::shared_ptr<SimpleSumeSwitchHandler>(new SimpleSumeSwitchHandler(sw));
}
